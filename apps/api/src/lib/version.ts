/** Engine rules + sampler used for cross-run pooling (excludes attribution). */
...
export function parseVersionTriple(
  ...
  if (!rulesRaw || !samplerRaw) {
    return {
      error:
        "rules_version and sampler_version are required so runs are never pooled across engine versions",
    };
  }
  ...
  return { rulesVersion, samplerVersion };
}
